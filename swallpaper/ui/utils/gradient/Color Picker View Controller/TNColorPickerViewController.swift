...
public final class TNColorPickerViewController: NSViewController {
    ...
    var saturationBrightnessView: TNSaturationBrightnessView!
    var hueSliderView: TNHueSliderView!
    var transparancySliderView: TNTransparancySliderView!
    var colorInformationView: TNColorInformationView!
    var colorPreviewCircle: TNColorPreviewCircleView!
    ...
    public init(colorMode: ColorMode, color: NSColor, delegate: TNColorPickerViewControllerDelegate? = nil) {
    ...
    public override func loadView() {
        ...
        containerView.edgeInsets = .init(top: 8, left: 8, bottom: 8, right: 8)
        containerView.orientation = .vertical
        
        saturationBrightnessView = .init()
        hueSliderView = .init()
        transparancySliderView = .init()
        colorInformationView = .init()
        colorPreviewCircle = .init(frame: .zero)
        
        containerView.snp.makeConstraints {
            $0.width.equalTo(250)
        }
                
        saturationBrightnessView.snp.makeConstraints {
            $0.height.equalTo(140)
        }
        
        hueSliderView.snp.makeConstraints {
            $0.height.equalTo(8)
        }
        
        transparancySliderView.snp.makeConstraints {
            $0.height.equalTo(8)
        }
        
        containerView.addArrangedSubview(saturationBrightnessView)
        containerView.addArrangedSubview(hueSliderView)
        containerView.addArrangedSubview(transparancySliderView)
        containerView.addArrangedSubview(colorInformationView)
...
#Preview {
    TNColorPickerViewController(colorMode: .rgb, color: .red)
}
